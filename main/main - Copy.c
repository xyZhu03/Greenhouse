...
#define APP_VERSION "v4.0.2" // pa test
...
static esp_err_t save_post_handler(httpd_req_t *req) {
    ...
    if (httpd_query_key_value(buf, "ssid", ssid, sizeof(ssid)) == ESP_OK &&
        ...
        esp_restart(); // Reinicia para conectar con los nuevos datos
    }
    return ESP_OK;
}
...
void start_ap_mode(void) {
    ...
    // Bucle infinito para mantener el AP vivo hasta que reinicien
...
void wifi_init_sta(void) {
    ...
    // USAR LAS VARIABLES GLOBALES CARGADAS DESDE NVS
...
void app_main(void) {
    ...
    // 2. Init Hardware básico
    init_gpio();
    init_i2c_bus();      // Inicializamos I2C antes para poder usar OLED en modo config
    init_oled_device();  // Init OLED

    // 3. LOGICA DE ARRANQUE: ¿Modo Configuración o Modo Normal?
    // Si se pulsa el botón al arrancar O NO hay WiFi guardado -> Modo AP
    ...
    if (boton_pulsado || !wifi_guardado) {
        ...
        start_ap_mode(); // Esta función bloquea y levanta el servidor web
        // El sistema se reiniciará desde la web cuando guardes datos
    }

    // 4. Si llegamos aquí, tenemos WiFi guardado y no se pulsó el botón
    esp_ota_mark_app_valid_cancel_rollback(); 
    cargar_estado_nvs(); // Cargar fases de cultivo, etc.
